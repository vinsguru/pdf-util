...
public class PDFUtil {
	...
	public int getPageCount(String file) throws IOException {
		...
		@Cleanup val doc = PDDocument.load(new File(file));
	...
	private String getPDFText(String file, int startPage, int endPage) throws IOException {
		...
		@Cleanup
		val doc = PDDocument.load(new File(file));

		val localStripper = null != this.stripper ? this.stripper : new PDFTextStripper();
		...
		var txt = localStripper.getText(doc);
	...
	private boolean comparepdfFilesWithTextMode(String file1, String file2, int startPage, int endPage)
			throws IOException {
		var file1Txt = this.getPDFText(file1, startPage, endPage).trim();
		var file2Txt = this.getPDFText(file2, startPage, endPage).trim();
		...
		val result = file1Txt.equalsIgnoreCase(file2Txt);
	...
	private List<String> saveAsImage(String file, int startPage, int endPage) throws IOException {
		...
		logger.info("startPage : " + startPage);
		logger.info("endPage : " + endPage);
		val imgNames = new ArrayList<String>();
		val sourceFile = new File(file);
		this.createImageDestinationDirectory(file);
		this.updateStartAndEndPages(file, startPage, endPage);
		val fileName = sourceFile.getName().replace(".pdf", "");
		@Cleanup
		...
		for (int iPage = this.startPage - 1; iPage < this.endPage; iPage++) {
			...
			val fname = FilenameUtils.concat(this.imageDestinationPath, fileName + "_" + (iPage + 1) + ".png");
			val image = pdfRenderer.renderImageWithDPI(iPage, 300, ImageType.RGB);
	...
	private boolean convertToImageAndCompare(String file1, String file2, int startPage, int endPage)
		...
		@Cleanup
		val doc1 = PDDocument.load(new File(file1));
		@Cleanup
		val doc2 = PDDocument.load(new File(file2));

		val pdfRenderer1 = new PDFRenderer(doc1);
		val pdfRenderer2 = new PDFRenderer(doc2);

		for (int iPage = startPage - 1; iPage < endPage; iPage++) {
			val fileName = FilenameUtils.getBaseName(file1);
			val absoluteFileName = FilenameUtils.concat(this.getImageDestinationPath(), fileName + "_diff.png");
			FileUtils.forceMkdirParent(new File(absoluteFileName));
			logger.info("Comparing Page No : " + (iPage + 1));
			val image1 = pdfRenderer1.renderImageWithDPI(iPage, 300, ImageType.RGB);
			val image2 = pdfRenderer2.renderImageWithDPI(iPage, 300, ImageType.RGB);
			result = ImageUtil.compareAndHighlight(image1, image2, absoluteFileName, this.bHighlightPdfDifference,
	...
	private void createImageDestinationDirectory(String file) throws IOException {
		if (null == this.imageDestinationPath) {
			val sourceFile = new File(file);
			val destinationDir = sourceFile.getParent() + "/temp/";
	...
	private void updateStartAndEndPages(String file, int start, int end) throws IOException {
		@Cleanup
		val document = PDDocument.load(new File(file));
